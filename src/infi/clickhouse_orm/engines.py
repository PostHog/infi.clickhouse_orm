...
class MergeTree(Engine):
    ...
    def __init__(
        ...
        assert date_col is None or isinstance(
            date_col, str
        ), "date_col must be string if present"
        ...
        assert (
            date_col or partition_key
        ), "You must set either date_col or partition_key"
        self.date_col = date_col
        self.partition_key = (
            partition_key if partition_key else ("toYYYYMM(`%s`)" % date_col,)
        )
    ...
    def create_table_sql(self, db):
        ...
        if db.server_version >= (1, 1, 54310):
            ...
            if self.primary_key:
                partition_sql += " PRIMARY KEY (%s)" % comma_join(
                    self.primary_key, stringify=True
                )
        ...
        # lei: for clickhouse cloud: no need to specify replica_table_path and replica_name
        return "%s %s" % (name, partition_sql)
